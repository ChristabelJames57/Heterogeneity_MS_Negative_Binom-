//Mhet model stan using Negative Binom observation 
functions {
...
parameters {
 ...
 // real<lower=0> phi_inv;//NEW FOR -VEBINOM
  real<lower=0> phi;
...
transformed parameters {
  ...
  //real phi = 1 / phi_inv;//ADDED FOR -VEBINOM remove since using phi
...
model {
  ...
  //slab_sd  ~ gamma(1,1);//Trial sensitvity after first NB
   slab_sd  ~ gamma(0.1, 0.1);//Trial sensitvity after first NB Again
 // slab_sd  ~ gamma(1,10);// old working normal and first NB
  theta_ss ~ beta(1, 1);                     // spike weight ~ Uniform(0,1)
  //phi ~ exponential(5);//go
  phi ~ exponential(1);//
  ...
  for (s in 1:n_states) {
    ...
    for (i in 1:(t_last[s] - 1)) {//add so it loops over i
     // cases[i, s] ~ normal( incidence[s, i],
      ...
      //cases[i, s] ~ normal( incidence[s, i],
                           // sqrt( incidence[s, i] * (1 - p_reported) ) );//global used for epidemic
    ...
    for (i in 1:(max_days - 1)) {
      if (i < t_last[s]) {
       ...
       // log_lik[s, i] = normal_lpdf( cases[i, s] |
                                        // incidence[s, i],
                                      //sqrt( incidence[s, i] * (1 - p_reported[s]) ) );
        //global epidemic used
        //pred_cases[s, i] = normal_rng( incidence[s, i],
                                     // sqrt( incidence[s, i] * (1 - p_reported ) ) );//global//epid used
      //  log_lik[s, i] = normal_lpdf( cases[i, s] | incidence[s, i],
                                    //sqrt( incidence[s, i] * (1 - p_reported ) ) );//epid used
